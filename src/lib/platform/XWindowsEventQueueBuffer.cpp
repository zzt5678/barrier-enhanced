...
static const int kPollTimeoutDelay = 25;
...
XWindowsEventQueueBuffer::waitForEvent(double dtimeout)
{
    ...
    const int timeout = (dtimeout < 0.0) ? -1 :
    ...
    // we want to give the cpu a chance so we cap this at 25ms.

    while (((dtimeout < 0.0) || (remaining > 0)) && getPendingCountLocked() == 0 && retval == 0) {
        const int timeoutDelay = pollTimeoutForTest(remaining);
#if HAVE_POLL
    retval = poll(pfds, 2, timeoutDelay);
    ...
    timeout.tv_sec = timeoutDelay / 1000;
    timeout.tv_usec = (timeoutDelay % 1000) * 1000;
    retval = select(nfds,
                        SELECT_TYPE_ARG234 &rfds,
                        SELECT_TYPE_ARG234 NULL,
                        SELECT_TYPE_ARG234 NULL,
                        SELECT_TYPE_ARG5   &timeout);
    if (FD_ISSET(m_pipefd[0], &rfds)) {
        read(m_pipefd[0], buf, 15);
    }
#endif
        remaining -= timeoutDelay;
    }
...
int
XWindowsEventQueueBuffer::pollTimeoutForTest(int remaining)
{
    if (remaining < 0 || remaining > kPollTimeoutDelay) {
        return kPollTimeoutDelay;
    }
    return remaining;
}
